...
export class ReservarClasePage {
  ...
  seleccionarProfesor(profesor: any){
    ...
    this.navCtrl.push(ReservarHoraPage, {'profesor': this.selecProf.profesorSeleccionado});
  }

  verDetalleProfesor(profesor: any, e: any){
    e.stopPropagation();
    this.selecProf.cargarProfesorSeleccionado(profesor);
    let modal = this.modalCtrl.create(ModalDetalleProfesorPage, {profesor: this.selecProf.profesorSeleccionado});
    modal.present();
  }
